...
try {
    $pdo = get_db();
    if ($_lang !== 'en') {
        $_ts = $pdo->prepare("SELECT content_id,field_name,field_value FROM content_translations WHERE content_type='news' AND language=?");
        $_ts->execute([$_lang]);
        foreach($_ts->fetchAll() as $_t) $_transMap[$_t['content_id']][$_t['field_name']] = $_t['field_value'];
    }
    $newsItems = $pdo->query(
        "SELECT n.*, u.full_name AS author_name FROM news n JOIN users u ON u.id = n.author_id WHERE n.status = 'published' ORDER BY n.published_at DESC, n.created_at DESC"
    )->fetchAll();
} catch (\Throwable $e) {
...
<img alt="A panoramic wide shot of a vibrant outdoor youth development rally" class="w-full h-full object-cover brightness-50 scale-105 animate-[pulse_10s_ease-in-out_infinite]" src="assets/news/01JV7D9W77JK90T9D9S63AACNE.jpg"/>
</div>
<div class="relative z-10 h-full flex flex-col justify-center items-center text-center px-margin-mobile reveal active">
<h1 class="font-display-lg text-display-lg text-white text-shadow-sm mb-base">News</h1>
...
<span class="font-bold"><?= __("nav_news") ?></span>
</nav>
</div>
</section>
<!-- Main Content -->
<main class="w-full max-w-7xl mx-auto px-margin-mobile md:px-margin-desktop py-xl">
<div class="text-center mb-xl">
<h2 class="font-headline-lg text-headline-lg text-on-surface mb-2"><?= __("home_news_heading") ?></h2>
<p class="text-on-surface-variant max-w-2xl mx-auto"><?= __("home_news_sub") ?></p>
</div>
<!-- News Feed - Grid Layout for Modern feel based on screenshot source -->
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-gutter" id="news-grid">
<?php if (!$newsItems): ?>
<p class="col-span-full text-center text-on-surface-variant py-12"><?= __("no_articles") ?></p>
<?php endif; ?>
<?php foreach ($newsItems as $n): ?>
<article class="news-card animate-fade-up bg-white rounded-lg border border-outline-variant overflow-hidden flex flex-col">
<div class="h-64 overflow-hidden">
<img class="w-full h-full object-cover" alt="<?= e($n['title']) ?>" src="<?= e($n['image'] ?: 'assets/news/aboutus-home.webp') ?>"/>
</div>
<div class="p-lg flex flex-col flex-grow">
<div class="flex items-center gap-2 mb-sm">
<span class="bg-tertiary-container text-on-tertiary-container px-2 py-1 rounded text-label-sm font-label-sm uppercase"><?= __("nav_news") ?></span>
<span class="text-on-surface-variant text-label-md font-label-md"><?= e(date('M d, Y', strtotime($n['published_at'] ?: $n['created_at']))) ?></span>
</div>
<h3 class="font-headline-md text-headline-md text-on-surface mb-md"><?= e(localized_content_value($_transMap[$n['id']]['title'] ?? $n['title'])) ?></h3>
<p class="text-on-surface-variant font-body-md text-body-md mb-lg line-clamp-3"><?= e(localized_content_value($_transMap[$n['id']]['excerpt'] ?? $n['excerpt'])) ?></p>
<div class="mt-auto">
<a href="article.php?slug=<?= e($n['slug']) ?>" class="inline-block border-2 border-primary text-primary px-6 py-2 font-bold rounded-lg hover:bg-primary hover:text-white transition-all active:scale-95"><?= __("read_more") ?></a>
</div>
</div>
</article>
<?php endforeach; ?>
...
<!-- Pagination -->
<div class="flex justify-center items-center gap-4 mt-xl animate-fade-up">
<button id="prev-page" class="w-10 h-10 flex items-center justify-center rounded-full border border-outline-variant text-on-surface hover:bg-surface-container transition-all">
<span class="material-symbols-outlined">chevron_left</span>
</button>
<div id="page-numbers" class="flex gap-2">
<!-- Dynamically populated by JS -->
</div>
<button id="next-page" class="w-10 h-10 flex items-center justify-center rounded-full border border-outline-variant text-on-surface hover:bg-surface-container transition-all">
...
<script>
    ...
    // Pagination Logic
    const ITEMS_PER_PAGE = 2; // Show 2 items per page
    const newsGrid = document.getElementById('news-grid');
    const articles = Array.from(newsGrid.querySelectorAll('article'));
    ...
    function showPage(page) {
        ...
        // Hide all articles and reset animation state
        ...
        // Show current page articles
        const activeArticles = articles.slice(start, end);
        activeArticles.forEach((article, index) => {
            article.classList.remove('hidden');
            // Trigger animation with a slight stagger delay
            setTimeout(() => {
        ...
        if (currentPage === 1) {
            ...
            prevBtn.classList.remove('hover:bg-surface-container');
        } else {
            prevBtn.disabled = false;
            prevBtn.classList.remove('opacity-50', 'cursor-not-allowed');
            prevBtn.classList.add('hover:bg-surface-container');
        }
        ...
        if (currentPage === totalPages) {
            ...
            nextBtn.classList.remove('hover:bg-surface-container');
        } else {
            nextBtn.disabled = false;
            nextBtn.classList.remove('opacity-50', 'cursor-not-allowed');
            nextBtn.classList.add('hover:bg-surface-container');
        }

        // Scroll back to news grid smoothly if user is scrolled past it
        const rect = newsGrid.getBoundingClientRect();
    ...
    function renderPageButtons() {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            btn.textContent = i;
            btn.className = "w-10 h-10 flex items-center justify-center rounded-full transition-all ";
            
            if (i === currentPage) {
                btn.className += "bg-primary text-white font-bold";
            } else {
                btn.className += "border border-outline-variant hover:bg-surface-container text-on-surface";
            }
