...
$id = (int)($_GET['id'] ?? 0);
$item = ['id' => 0, 'title' => '', 'category' => 'exhibitions', 'media_type' => 'image', 'file_path' => '', 'video_url' => '', 'status' => 'draft', 'author_id' => $user['id'], 'review_note' => null];

if ($id) {
    $stmt = $pdo->prepare('SELECT * FROM gallery_items WHERE id = ?');
    $stmt->execute([$id]);
    $found = $stmt->fetch();
    if (!$found) { flash('error', 'Item not found.'); header('Location: gallery.php'); exit; }
    if (!can_edit_item($user, $found)) { flash('error', "You don't have permission to edit that item."); header('Location: gallery.php'); exit; }
    $item = $found;
}

$categories = ['exhibitions', 'youth-day', 'vtc', 'ysz', 'videos'];
$errors = [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();
    $title = trim($_POST['title'] ?? '');
    $category = $_POST['category'] ?? 'exhibitions';
    ...
    try {
        $uploaded = handle_upload('file', 'gallery', ['jpg', 'jpeg', 'png', 'webp'], 8);
        if ($uploaded) {
    ...
    if (!$filePath) $errors[] = 'Please upload a cover image for this item.';

    if (!$errors) {
        $status = resolve_save_status($action, $user);

        if ($id) {
            $sql = "UPDATE gallery_items SET title=?, category=?, media_type=?, file_path=?, video_url=?, status=?, updated_at=NOW()";
            $params = [$title, $category, $mediaType, $filePath, $videoUrl ?: null, $status];
            if ($status === 'published') { $sql .= ", reviewed_by=?, review_note=NULL"; $params[] = $user['id']; }
            $sql .= " WHERE id=?"; $params[] = $id;
            $pdo->prepare($sql)->execute($params);
            log_activity($user, 'updated gallery item', $title);
        } else {
            $sql = "INSERT INTO gallery_items (title, category, media_type, file_path, video_url, status, author_id" . ($status === 'published' ? ', reviewed_by' : '') . ") VALUES (?,?,?,?,?,?,?" . ($status === 'published' ? ',?' : '') . ")";
            $params = [$title, $category, $mediaType, $filePath, $videoUrl ?: null, $status, $user['id']];
            if ($status === 'published') $params[] = $user['id'];
            $pdo->prepare($sql)->execute($params);
            log_activity($user, 'added gallery item', $title);
        }
...
$pageTitle = $id ? 'Edit Gallery Item' : 'Add Gallery Item';
$activeNav = 'gallery';
require __DIR__ . '/includes/header.php';
?>

<a href="gallery.php" class="text-sm text-on-surface-variant hover:text-primary flex items-center gap-1 mb-4">
...
<form method="post" enctype="multipart/form-data" class="bg-white border border-outline-variant rounded-xl p-6 max-w-2xl">
  ...
  <label class="block text-sm font-medium mb-1">Title / caption</label>
  ...
  <div class="grid grid-cols-2 gap-4 mb-4">
    <div>
      ...
      <select name="category" class="w-full rounded-lg border border-outline-variant px-4 py-2.5">
        <?php foreach ($categories as $cat): ?>
          <option value="<?= e($cat) ?>" <?= $item['category'] === $cat ? 'selected' : '' ?>><?= e(str_replace('-', ' ', $cat)) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div>
      <label class="block text-sm font-medium mb-1">Media type</label>
      <select name="media_type" class="w-full rounded-lg border border-outline-variant px-4 py-2.5">
  ...
  <label class="block text-sm font-medium mb-1">Video URL <span class="text-on-surface-variant font-normal">(only if media type is Video, e.g. a YouTube link)</span></label>
  <input name="video_url" value="<?= e($item['video_url']) ?>" class="w-full mb-4 rounded-lg border border-outline-variant px-4 py-2.5 focus:ring-2 focus:ring-primary outline-none">

  <label class="block text-sm font-medium mb-1">Cover image</label>
  <?php if ($item['file_path']): ?><img src="../<?= e($item['file_path']) ?>" class="h-28 rounded-lg mb-2 object-cover"><?php endif; ?>
  <input type="file" name="file" accept=".jpg,.jpeg,.png,.webp" class="w-full mb-6 text-sm">
  ...
  <div class="flex flex-wrap gap-3 pt-4 border-t border-outline-variant items-center">
    ...
    <?php if ($id): ?>
      <a href="preview.php?type=gallery&id=<?= $id ?>" target="_blank"
